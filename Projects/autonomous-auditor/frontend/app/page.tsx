...
export default function Home() {
  ...
  const fetchHistory = () => {
    // UPDATED: Relative path for secure Nginx routing
    fetch('/api/history')
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    fetch('/api/status')
      ...
      .catch(err => console.error("Could not connect to The Brain", err));
    fetchHistory();
  }, []);
  ...
  const startAudit = async () => {
    ...
    try {
      // UPDATED: Relative path for secure Nginx routing
      const res = await fetch(`/api/scan?repo_url=${encodeURIComponent(targetUrl)}`, { method: 'POST' });
