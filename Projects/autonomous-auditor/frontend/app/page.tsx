...
export default function Home() {
  ...
  // NEW: Function to clear the terminal completely
  ...
  const startAudit = async () => {
    ...
    // Basic validation to make sure it looks like a URL
    ...
    try {
      ...
      if (!res.ok) {
          throw new Error("The Brain rejected the request.");
      }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-8 bg-gray-900 p-4 rounded-xl border border-gray-800 flex gap-4 shadow-lg focus-within:border-blue-500/50 transition-colors">
          ...
          <button 
            ...
            disabled={isScanning}
            className="bg-red-950/30 hover:bg-red-900/50 text-red-400 border border-red-900/50 font-bold py-3 px-6 rounded-lg transition-all disabled:opacity-50"
          >
            Clear
          </button>
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-lg shadow-[0_0_15px_rgba(37,99,235,0.3)] hover:shadow-[0_0_25px_rgba(37,99,235,0.5)] disabled:opacity-50 disabled:shadow-none transition-all flex items-center gap-2"
          >
            {isScanning ? (
            ...
            {scanResults.map((result, index) => (
              <div key={index} className="bg-[#0a0a0a] border border-gray-800 p-6 rounded-xl shadow-2xl font-mono overflow-x-auto animate-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${index * 150}ms`, animationFillMode: 'both' }}>
                ...
                <div className="text-gray-300 space-y-3">
                  ...
                  <div className={`mt-4 p-4 rounded-lg border-l-2 ${result.vulnerabilities === 0 ? 'bg-green-950/20 border-green-500 text-green-300' : 'bg-red-950/20 border-red-500 text-red-300'}`}>
                    {result.report}
                  </div>
                </div>
              </div>
